import { AppEntity, LocalRepositoryNames, LogCategory } from '@constants';
	...
	async update(id: UUID, item: Partial<T>): Promise<number> {
		...
		await this.db.transaction('rw', [
			...
			if (entity) {
				...
				// count = await this.db.table<T>(this.appEntity.table).where(":id").equals(id).modify( { ...entity, ...item });
